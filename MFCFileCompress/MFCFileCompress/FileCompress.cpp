...
void FileCompress::thread0(const std::string & FilePath)
{
...
void FileCompress::Compress(const std::string &FilePath)
{
	...
	CompressFileName += ".gl";
	CompressFileName = "E:\\" + CompressFileName;*/
	...
	if (NULL == output)
	{
		std::cout << FileName << " Can Not Be Create !" << std::endl;
		system("pause");
		exit(2);
	}
	...
	//计算压缩率
	long size = _filelength(_fileno(input));
	long size1 = _filelength(_fileno(output));
	double ratio = 100.0*size1 / size;
	wz_ratio = new wchar_t[64];
	swprintf(wz_ratio,64, L"%.2f", ratio);

	fclose(input);
	fclose(output);
}

wchar_t * FileCompress::GetWz_ratio()
{
	return wz_ratio;
}

void FileCompress::thread1(const std::string & FilePath)
{
	this->UnCompress(FilePath);
}
...
void FileCompress::UnCompress(const std::string &FilePath)
{
	...
	if (Postfix.find("~") != Postfix.npos)      //找得到符号“~”
	{
		//获取密码
		std::string password1;
		password1 = Postfix.substr(Postfix.find("~") + 1);
		Postfix = Postfix.substr(0, Postfix.find("~"));
		INT_PTR nRes;             // 用于保存DoModal函数的返回值   
		CPasswordDlg passwordDlg;           // 构造对话框类CTipDlg的实例   
		while (true)
		{
			nRes = passwordDlg.DoModal();  // 弹出对话框 
			USES_CONVERSION;
			std::string password2(W2A(passwordDlg.strPassword));
			if (IDCANCEL != nRes)
			{
...
void FileCompress::SetPassword(std::string passwor)
{
	password = passwor;
}
