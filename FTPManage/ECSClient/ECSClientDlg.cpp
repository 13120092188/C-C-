...
/*	BOOL bRet = FALSE;
	//随机产生一个可用的端口号
create:
	bRet = m_ClientSock.Create(3000+rand()%2000,SOCK_STREAM,chIP);
	if (bRet==FALSE)
		goto create;

connect:	//连接服务器
	bRet = m_ClientSock.Connect("192.168.1.23",123);
	if (bRet==FALSE)
		goto connect;
*/
...
void CECSClientDlg::LoadOrgFromFile(BSTR csFileName)
{
	...
	childNode = m_pXMLDoc->selectSingleNode("//ITEM[@部门ID = 0]");	//查找根节点
	if (childNode != NULL)
	{
...
void CECSClientDlg::FindSubNode(IXMLDOMElementPtr pParentNode ,HTREEITEM hParentItem)
{
	if (pParentNode != NULL)
	{
		VARIANT varVal; 
		...
		else 
		{
			...
			if (csSex=="男")
			{
				dwData = MAKELPARAM(SEX_MAN,UserID);//设置帐户数据
			}
			else if (csSex=="女")
			{
				dwData = MAKELPARAM(SEX_WOMAN,UserID);//设置帐户数据
			}
			
			/*
			if (csSex=="男")
				hRoot = m_ACList.InsertItem(csText,1,1,hParentItem);
			else if (csSex=="女")
				hRoot = m_ACList.InsertItem(csText,2,2,hParentItem);
			*/
		}	
		m_ACList.SetItemData(hRoot,dwData);
	//	m_ACList.SetItemData(hRoot,dwData);

		IXMLDOMElementPtr pNode = pParentNode->GetfirstChild();
		while(pNode != NULL)
		{	
			//递归调用
...
void CECSClientDlg::OnDblclkAclist(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	if (hItem != NULL)	
	{
		...
		//对于当前登录的帐户，不在发送数据的范围内，即不能够向自己发送数据
		if ((lowID==SEX_MAN || lowID==SEX_WOMAN) && ID != 0 && ID != m_nLoginUser)
		{
			CSendDlg* pDlg = NULL;
			POSITION pos = NULL;
...
BOOL CECSClientDlg::FindUserName(int nUserID, CString &csName)
{
	...
	if (hRoot != NULL)
	{
	//	if (m_ACList.ItemHasChildren(hRoot))	//如果根节点下有子节点，遍历所有子节点
	//	{
			//HTREEITEM hSubItem = m_ACList.GetChildItem(hRoot);	//获取第一个子节点
			HTREEITEM hSubItem;
			return  FindNode(hRoot,nUserID,csName,hSubItem);

			//	while(hSubItem != NULL)
		//	{
		//		FindNode(hSubItem,nUserID,csName);
...
BOOL CECSClientDlg::FindNode(HTREEITEM hParent, int nUserID, CString &csName,HTREEITEM &hRetItem)
{
	//先判断是否为帐户节点
	//对于帐户节点来说，低字节为0，高字节表示帐户ID
...
void CECSClientDlg::OnTimer(UINT nIDEvent) 
{
	if (nIDEvent == 1)
	{
		if (m_nTestNum > 8)	//8*2秒种没有收到服务器发来的信息，表示已经断线
		{
			m_nTestNum = 0;
