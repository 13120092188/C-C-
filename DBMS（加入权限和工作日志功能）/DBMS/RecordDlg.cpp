...
//	ON_WM_UNICHAR()
ON_BN_CLICKED(IDC_BUTTON1, &RecordDlg::OnBnClickedButton1)
END_MESSAGE_MAP()
...
BOOL RecordDlg::OnInitDialog()
{
	...
	m_listCtrl.InsertColumn(0, CString("字段名称"), LVCFMT_LEFT, 100);
	m_listCtrl.InsertColumn(1, CString("字段类型"), LVCFMT_LEFT, 100);
	m_listCtrl.InsertColumn(2, CString("主键"), LVCFMT_LEFT, 100);
	m_listCtrl.InsertColumn(3, CString("为空"), LVCFMT_LEFT, 100);
	newRecord = new CString[totalNumber];
	std::vector<Field> v;
	TableEntity te;
	te.getAttributes(SDBName, STBName, v);
	totalNumber = v.size();
	rowsNum = totalNumber + 2;
	...
	m_listCtrl.InsertItem(totalNumber, L"字段名称");
	for (int i = 1; i <= totalNumber; i++)
	{
		m_listCtrl.SetItemText(totalNumber, i, v[i - 1].m_FieldName);
	}

	m_listCtrl.InsertItem(totalNumber + 1, CString("数值"));
	//
...
void RecordDlg::OnBnClickedOk() // 确定
{
	...
	TableEntity te;
	te.getAttributes(SDBName, STBName, v);
	for (int j = totalNumber + 1; j < rowsNum; j++)
	{

		for (int i = 0; i < v.size(); i++)
		{

			if (m_listCtrl.GetItemText(i, 2) == L"Y")
			{
				for (int n = 0; n < rnum; n++)
				{
					std::map<CString, CString> tmp = vct[n];

					if (m_listCtrl.GetItemText(j, i + 1) == tmp[v[i].m_FieldName] || m_listCtrl.GetItemText(j, i + 1) == "")
					{
						allright = false;
			...
			if (m_listCtrl.GetItemText(i, 3) == L"Y")
			{
				if (m_listCtrl.GetItemText(j, i + 1) == "")
				{
					allright = false;
		...
		if (allright == true)
		{
			for (int i = 0; i < v.size(); i++)
			{
				CString str1 = m_listCtrl.GetItemText(j, i + 1);
				//newRecord->Append(str1);
				//newRecord[i] = m_listCtrl.GetItemText(totalNumber + 2, i + 1);
				record.insert(std::make_pair(v[i].m_FieldName, str1));
			}
			RecordEntity re;
			//int x1 = record.size();
		...
		else
		{
			for (int i = 0; i < v.size(); i++)
			{
				m_listCtrl.SetItemText(j, i + 1, L"");
			}
		}
	}
...
void RecordDlg::OnBnClickedButton1()
{
	// TODO:  在此添加控件通知处理程序代码
	m_listCtrl.InsertItem(rowsNum, CString("数值"));
	rowsNum++;
}
