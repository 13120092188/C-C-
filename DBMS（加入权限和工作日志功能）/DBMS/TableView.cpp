...
void CTableView::DisplayField()
{
	this->ClearTable();
	// 初始界面查看表结构的表格头
	listCtrl->InsertColumn(0, _T("字段名"), LVCFMT_LEFT, 100);
	listCtrl->InsertColumn(1, _T("字段类型"), LVCFMT_LEFT, 100);
	listCtrl->InsertColumn(2, _T("长度"), LVCFMT_LEFT, 80);
	listCtrl->InsertColumn(3, _T("主键"), LVCFMT_LEFT, 90);
	listCtrl->InsertColumn(4, _T("非空"), LVCFMT_LEFT, 90);
	listCtrl->InsertColumn(5, _T("默认"), LVCFMT_LEFT, 80);
	listCtrl->InsertColumn(6, _T("最大值"), LVCFMT_LEFT, 80);
	listCtrl->InsertColumn(7, _T("最小值"), LVCFMT_LEFT, 80);
...
void CTableView::OnRecordAdd()
{
	...
	TableEntity te;
	te.getAttributes(SDBName, STBName, v);
	int x = (dlg.rowsNum - dlg.totalNumber - 1) * 2;
